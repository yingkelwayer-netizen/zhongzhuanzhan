...
const renderFooter = (footerLinks) => {
  return footerLinks.map((f, i) => `<a href="${f.url}" class="footer-link">${f.name}</a>${i < footerLinks.length - 1 ? ' &middot;' : ''}`).join('\n          ');
};
