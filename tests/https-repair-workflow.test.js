...
assert.match(workflow, /location = \/index\.html \{\s*if \(\$request_uri ~ "\^\/index\\\.html\(\?:\\\?\.\*\)\?\$"\) \{\s*return 301 https:\/\/zonghuin\.com\/;/, 'Only a direct /index.html request may redirect, so the homepage index resolution cannot loop');
assert(workflow.includes('index_https_headers='), 'Runtime verification must request /index.html');
assert(workflow.includes('root_https_headers='), 'Runtime verification must request the canonical homepage');
assert(workflow.includes('"$root_https_headers" | grep -Eq \'^HTTP/[^ ]+ 200\''), 'Runtime verification must require a 200 response from the canonical homepage');
